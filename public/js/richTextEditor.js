$(document).ready(function () {
  ...
  $(".droppable-area1").sortable({
    ...
    stop: function (event, ui) {
      ...
      for (let x = 1; x <= trLength; x++) {
        ...
        $(`.courses-tbody li:nth-child(${x})`).find("input.input-lessonnumber").val(`${x}`)
        $(`.courses-tbody li:nth-child(${x})`).find("input.input-lessonnumber").attr("value", `${x}`);
        // 變更 button 的 data-lessonnumber(同章節編號)
        $(`.courses-tbody li:nth-child(${x})`).find("button").attr("data-lessonnumber", `${x}`);
        ...
        axiosReq.push(axios.post(`/courses/create/${courseId}/step2/editLessonNumber?_method=PUT`, {
          pk: pk,
  ...
  const modifyRowNumber = async function (trLength) {
    ...
    for (let x = 1; x <= trLength; x++) {
      ...
      // 取得 li 的 input 之 data-pk 值
      let pk = $(`.courses-tbody li:nth-child(${x})`).find("input").data('pk')

      // 取得 li 的 input value
      let lessonNumber = $(`.courses-tbody li:nth-child(${x})`).find("input").val()
      // 取得 li 的 input 之 data-courseId 值
      let courseId = $(`.courses-tbody li:nth-child(${x})`).find("input").data('courseid')
  ...
  $(".courses-tbody").on('click', 'button.btn-remove-lesson', function () {
    if (confirm('Some message')) {
      ...
      // 取得 li 的 input value
      let lessonNumber = $('.courses-tbody').children().find('button.btn-remove-lesson').data('lessonnumber')
