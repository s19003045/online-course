...
const courseController = {
  ...
  postCourseStep2: (req, res) => {
    const { lessonNumber, intro, title, contents, totalTime, isPreview } = req.body
    // 檢查必填欄位是否已填寫(若有提供暫存草稿機制，則不用檢查欄位。直到送出申請前再檢查)
    if (!lessonNumber || !intro || !title || !contents || !totalTime || !isPreview) {
              ...
              .then(lessons => {
                Course.findByPk(req.params.courseId)
                  .then(course => {
                    ...
                    course.save()
                      .then(course => {
                        return res.render('createCourseStep2', { lessons, course, lesson })
                      })
                  })
              })
          })
  ...
  createCourseStep3: (req, res) => {
    Course.findByPk(req.params.courseId, { include: [Lesson] })
      .then(course => {
  ...
  postCourseStep4: (req, res) => {
    ...
    Course.findByPk(req.params.courseId, {
      ...
      .then(course => {
        const { name, description, introVideo, teacherName, teacherDescrip, totalTime, totalLessons, price } = course
